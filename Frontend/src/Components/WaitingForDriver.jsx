import React from 'react'

const WaitingForDriver = ({setwaitingForDriver , waitingForDriverRef , ride}) => {
  return (
        <div >
        <div ref={waitingForDriverRef} className='fixed w-full z-10 bottom-0 pl-3 pb-3 bg-white flex flex-col gap-2 translate-y-full'>
            <h5 onClick={()=>{setwaitingForDriver(false) ,  setmap(true)}}
                className='w-full text-center  text-2xl'><i className="ri-arrow-down-wide-line text-3xl"></i></h5>
            <div className='flex items-center justify-between pr-5'>
                <img className='h-10 ' src="data:image/jpeg;base64," />
              <div className='text-right'>
                <h2 className='text-lg font-medium capitalize'>{ride?.captain.fullname.firstname + ride?.captain.fullname.lastname }</h2>
                <h4 className='text-md font-semibold'>{ride?.captain.vehicle.plate}</h4>
                <p className='text-sm  text-gray-600 capitalize'>{ride?.captain.vehicle.vehicleType}</p>
                <h1 className='text-lg font-bold text-gray-700  '>OTP: {ride?.otp}</h1>
              </div>
            </div>
            <div className='p-3 flex flex-col gap-3'>
                <div className='flex items-center  border-gray-300 border-b-3'>
                    <h4 className='w-[15%]'>
                        <i className="ri-map-pin-range-fill text-2xl"></i>
                    </h4>
                    <div className='flex flex-col w-[80%]'>
                        {/* <h5 className='text-xl font-bold'></h5> */}
                        <p className='text-lg  text-gray-600'>{ride?.pickup}</p>
                    </div>
                </div>
                <div className='flex items-center  border-gray-300 border-b-3'>
                    <h4 className='w-[15%]'>
                        <i className="ri-square-fill text-2xl"></i>
                    </h4>
                    <div className='flex flex-col w-[80%]'>
                        {/* <h5 className='text-xl font-bold'>Third Cafe Cofee</h5> */}
                        <p className='text-lg text-gray-600 '>{ride?.destination}</p>
                    </div>
                </div>
                <div className='flex items-center border-gray-300 border-b-3'>
                    <h4 className='w-[15%]'>
                        <i className="ri-cash-line text-2xl"></i>
                    </h4>
                    <div className='flex flex-col w-[80%]'>
                        <h5 className='text-xl font-bold'>{ride?.fare}</h5>
                        <p className='text-lg  text-gray-600'>Cash Cash</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
  )
}

export default WaitingForDriver